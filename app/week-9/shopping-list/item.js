// Part 2: The Components
// Build the Item Component
// In item.js, create a functional component named Item.
// This component must accept name, quantity, and category as props. It should render these details in a list item (<li>) element, styled with Tailwind CSS to look organized (e.g., using a card-like appearance).
// Tip: Ensure you are destructuring the props in the function arguments or accessing them via a props object.
export default function Item({ name, quantity, category, onSelect }) {
    const item = { name, quantity, category }; // build the item object

    return (
        <li
            onClick={() => {
                console.log("Item clicked:", name);
                if (typeof onSelect === "function") {
                    onSelect(item); // pass the item here
                }
            }}
            className="bg-green-100 p-4 rounded-lg shadow-md border border-gray-200 mb-3 dark:bg-gray-600 dark:border-gray-900"
        >
            <p className="text-lg font-semibold text-pink-400">{name}</p>
            <p className="text-sm text-purple-400">Quantity: {quantity}</p>
            <p className="text-sm text-purple-400">Category: {category}</p>
        </li>
    )
}


/*export default function Item({ name, quantity, category, onSelect }) {
    return (
        // <li onClick = {onSelect} 
        <li onClick = {()=> {
            console.log("Item clicked:", name);
            if (typeof onSelect === "function") {
                onSelect();
            }
        }}
            className="bg-green-100 p-4 rounded-lg shadow-md border border-gray-200 mb-3 dark:bg-gray-600 dark:border-gray-900">
            <p className="text-lg font-semibold text-pink-400">{name}</p>
            <p className="text-sm text-purple-400">Quantity: {quantity}</p>
            <p className="text-sm text-purple-400">Category: {category}</p>
        </li>
    )
}*/

